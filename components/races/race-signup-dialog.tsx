...
import { createCompApplication } from "@/app/actions/comp-application";
import type { Race, Member, CompApplication } from "@/api/types";
...
type Props = {
	...
	onSubmitted: (participant: CompApplication) => void;
};
...
export default function RaceSignupDialog({
	...
	const handleVerify = useCallback(() => {
		...
		const found = members.find((m) => {
			if (m.full_name.trim() !== name.trim()) return false;
			return normalizeBirthDate(m.birthday) === inputNorm;
		});

		if (!found) {
			setVerifyError(
				"등록된 회원 정보와 일치하지 않습니다. 이름과 생년월일을 확인해주세요.",
			);
			return;
		}
		if (found.status !== "active") {
			setVerifyError(
				"탈퇴한 회원입니다. 문의사항은 운영진에게 연락해주세요.",
			);
			return;
		}
	...
	// 코스 목록 구성
	const courseOptions = race.courses.map((c) => c.competition_class);
	...
	const existingEntry = race.participants
		.filter((p) => p.full_name === name.trim())
		.at(-1);

	// Step 2: 코스 선택 후 제출
	const handleSubmit = useCallback(async () => {
		if (!selectedCourse || !verifiedMember) return;
		setSubmitting(true);

		const matchedCourse = race.courses.find(
			(c) => c.competition_class === selectedCourse,
		);

		try {
			const result = await createCompApplication({
				member_id: verifiedMember.id,
				full_name: sanitizeText(name, 20),
				competition_id: matchedCourse?.competition_id || "",
				competition_name: race.name,
				competition_class: selectedCourse,
			...
			alert(
				`제출 중 오류가 발생했습니다: ${err instanceof Error ? err.message : err}`,
			);
		} finally {
			setSubmitting(false);
		}
		...
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>{race.date}</DialogDescription>
				</DialogHeader>
					...
					<div className="space-y-4">
						...
						{existingEntry && (
							<p className="rounded bg-yellow-900/30 border border-yellow-700/30 px-3 py-2 text-sm text-yellow-300">
								이미{" "}
								<strong>
									{existingEntry.competition_class}
								</strong>
								(으)로 신청되어 있습니다. 다시 신청하면 마지막
								신청으로 변경됩니다.
							</p>
						)}
